...
router.post(
	...
	body(["eventName", "date"])
		.exists()
		.notEmpty()
		.withMessage("Fields are missing"),
	body("eventName")
		.isLength({ min: 5, max: 30 })
		.withMessage(
			"Event name must be at least 5 characters, but not longer than 30"
		),
	body("date").isISO8601({ strict: true, strictSeparator: true }),
	body("date").isAfter().withMessage("Event's date must be in the future"),
	body("date")
		.isBefore(today(10))
		.withMessage("Event's date must not be so far in the future"),
...
router.get("/event/:eventId", loggedInMiddleware, async (req, res) => {
	const eventId = Number(req.params.eventId);
	try {
		...
		if (!guestList)
			return res
				.status(204)
				.send(
					`Event with id: ${eventId} does not exists or has no guests yet.`
				);
		res.status(200).send({
			guestList,
		});
...
router.get("/event/:eventId/notIn", loggedInMiddleware, async (req, res) => {
	const eventId = Number(req.params.eventId);
	try {
		const guestListNotIn = await EventGuestList.getGuestsNotInList(eventId);
		if (!guestListNotIn)
			return res
				.status(204)
				.send(
